...
from regularization import l1_kernel_prox, l1_prox
...
def main():
  output_file = 'resnet56_sparse_model_cosine_7en7.dat'
  batch_size = 128
  epoch_count = 450
  ...
  # conv_net = resnet56().cuda() # use when run with GPU
  conv_net = resnet56()
  conv_net.train()
  ...
  #                                 init_step_size=init_lr, mom_ts=12.37, b_mom_ts=12.37, weight_decay=5e-4, init_av_param=av_param)

  training_specs = IterationSpecs(step_size=init_lr, mom_ts=12.37, b_mom_ts=12.37, weight_decay=5e-4, av_param=av_param)
